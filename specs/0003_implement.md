...
> 目标：使用指定前端技术栈完成一个专业、美观、可演示的静态后台 Demo。  
> 客户行业：汽配行业。所有页面与 mock 数据均采用汽配制造集团场景。

## 1. 实施目标

本前端 Demo 用于展示资产管理系统一期需求的完整业务闭环。Demo 不连接真实后端，所有数据通过本地 mock 文件提供，并通过 Axios 适配层模拟接口请求。
...
Demo 中所有模块必须有可见内容，不能出现空页面、空表格或占位式页面。
...
- TanStack Query：列表、详情、看板、模拟接口数据请求状态。
...
| 路由 | 页面 | 主要内容 | 权限标识 |
| --- | --- | --- | --- |
| `/login` | 登录页 | 角色选择、模拟登录 | `public` |
| `/dashboard` | 首页看板 | 资产总览、预警、待办、趋势图 | `dashboard:view` |
| `/assets/list` | 固定资产台账 | 资产列表、查询、详情、导入演示 | `asset:view` |
...
Demo 面向汽配制造集团，界面应体现“专业、清晰、稳定、可运营”：
...
- 操作：查看详情、编辑、查看档案、发起调拨、发起处置。

覆盖需求：FR-001 至 FR-009。
...
- 操作：查看、确认入账、补充档案、导入演示。

覆盖需求：FR-010、FR-053 至 FR-060。
...
覆盖需求：FR-012、FR-013、FR-017、FR-028、FR-029。
...
- 操作：新增备件、编辑、查看详情、查看库存、查看适用设备。

覆盖需求：FR-031、FR-039、FR-040。
...
| 需求编号 | 页面/模块 |
| --- | --- |
| FR-001 至 FR-009 | 固定资产台账、资产入账、资产盘点 |
| FR-010 至 FR-017 | 资产入账、资产调拨、资产处置、资产详情 |
| FR-018 至 FR-025 | 一机一档、档案附件、资产详情 |
| FR-026 至 FR-030 | 组织架构、多厂区查询、资产调拨、数据权限 |
| FR-031 至 FR-040 | 备件台账、库存管理、入库、出库、盘点、预警 |
| FR-041 至 FR-046 | MOM 数据展示 |
| FR-047 至 FR-052 | 数据看板、资产分析、厂区对比、维修分析、费用分析、备件分析 |
| FR-053 至 FR-060 | MOM 对接、财务对接、项目管理对接、资产入账 |
